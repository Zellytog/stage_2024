...
\begin{defi}[Typage]
  ...
  \begin{center}
    ...
    \begin{prooftree}
      ...
      \infer1[Aff$_1$]{\Gamma, \bx : S\mid\Delta\mid\Xi \vdash t : \varphi}
    \end{prooftree}
    \quad
    \begin{prooftree}
      \hypo{\Gamma\mid\Delta\mid\Xi\vdash t : \varphi}
      \infer1[Aff$_2$]{\Gamma\mid\Delta, X : S_1,\ldots,S_n\mid\Xi\vdash t : \varphi}
    \end{prooftree}
    \quad
    \begin{prooftree}
      \hypo{\Gamma\mid\Delta\mid\Xi\vdash t : \varphi}
    ...
    \begin{prooftree}
      \infer0[$\bN_\mathrm e$]{\Gamma\mid\Delta, X : \Nat\mid\Xi\vdash \rec_\bN : X(\bZ) \to (\forall \bx^{\{\Nat\}}, X(\bx) \to X(\bfS\;\bx)) \to \forall \bx^{\{\Nat\}}, X(\bx)}
    \end{prooftree}
    ...
    \begin{prooftree}
      \infer0[$\bL_\mathrm e$]{\Gamma\mid\Delta, X : \List\mid\Xi\vdash \rec_\bL : X(\bnil) \to (\forall \bn^{\{\Nat\}}\;\bx^{\{\List\}}, X(\bx) \to X(\bn \bcons \bx)) \to \forall \bx^{\{\List\}},X(\bx)}
    \end{prooftree}
...
\begin{proof}
  ...
  \item On veut montrer que $\rec_\bN \reali_\rho^\sigma X(\bZ) \to (\forall \bn^{\{\Nat\}}, X(\bn) \to X(\bfS\;\bn))\to \forall \bn^{\{\Nat\}}, X(\bn)$. Soient donc $t,u,v\in\Lambda$ tel que $t^\nu \reali_\rho^\sigma X(\bZ)$, $u^\nu \reali_\rho^\sigma \forall \bn^{\{\Nat\}}, X(\bn) \to X(\bfS\;\bn)$ et $v^\nu \reali_\rho^\sigma \Nat(\bn)$, montrons que $\rec_\bN\;t\;u\;v\reali_\rho^\sigma X(\bn)$. Par définition de $v^\nu\reali_\rho^\sigma \Nat(\bn)$, on sait que $v^\nu \reduc^* S^m\;0$ pour un certain $m\in\bN$. On raisonne par induction sur ce $m$~:
    \begin{itemize}
    \item si $v^\nu\reduc^* 0$, alors
      \begin{align*}
        (\rec_\bN\;t\;u\;v)^\nu &= \rec_\bN\;t^\nu\;u^\nu\;v^\nu\\
        &\reduc^* \rec_\bN\;t^\nu\;u^\nu\;0\\
        &\reduc t^\nu \reali_\rho^\sigma X(\bZ)
      \end{align*}
      donc par saturation $(\rec_\bN\;t\;u\;v)\reali_\rho^\sigma X(\bn)$ dans le cas où $\bn^\sigma = 0$.
    \item si $v^\nu\reduc^* S\;n'$ et, par hypothèse d'induction, que pour tout $w\in\Lambda$, $w \reduc^* n' \implies (\rec_\bN\;t\;u\;w)^\nu\reali_\rho^\sigma X(n')$, alors
      \begin{align*}
        (\rec_\bN\;t\;u\;v)^\nu&= \rec_\bN\;t^\nu\;u^\nu\;v^\nu\\
        &\reduc^* \rec_\bN\;t^\nu\;u^\nu\;(S\;n')\\
        &\reduc^* u^\nu\;n'\;(\rec_\bN\;t^\nu\;u^\nu\;n')
      \end{align*}
      or comme $u^\nu\reali_\rho^\sigma \Nat(n') \to X(n') \to X(\bfS\;n')$ et par hypothèse d'induction, on en déduit par saturation que $(\rec_\bN\;t\;u\;v)^\nu\reali_\rho^\sigma X(\bfS\;n')$
    \end{itemize}
    Donc par induction, pour tout $\bn : \Nat$ et $v\reali_\rho^\sigma \Nat(\bn)$, on a $(\rec_\bN\;t\;u\;v)^\nu\reali_\rho^\sigma X(\bn)$, donc $\rec_\bN\reali_\rho^\sigma X(\bZ) \to (\forall \bn^{\{\Nat\}}, X(\bn) \to X(\bfS\;\bn)) \to \forall \bn^{\{\Nat\}}, X(\bn)$.
